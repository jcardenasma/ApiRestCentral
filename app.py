...
# Base de datos
DATABASE_URL = os.environ['HEROKU_POSTGRESQL_WHITE_URL']
...
def login():
    ...
    data = request.get_json()
    print("Cuerpo:" ,data)
    busqueda = Cliente.query.filter(Cliente.username == str(data['rfc']), Cliente.password == data['password']).first_or_404()
